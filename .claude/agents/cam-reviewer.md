...
## Machine contract (validated vs the machine reference .nc — do not drift)
...
## Cutting pattern (the production pattern — production defaults in `tpDefaults`)
...
- Never approve changes to header/toolchange/footer format, CRLF, modality, datum signs, or the production cutting pattern (lastPass/ramp defaults) without the user explicitly confirming against the machine. For anything touching Z, feeds, or datum: recommend an air-cut (Z offset up) dry run before real material.
