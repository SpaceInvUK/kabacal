...
- **Explainability**: the wall inspector shows, dynamically, e.g. "measured 2000 → panel 1956 (panel 22 · frame 80) / Start — Butt corner: panel stops short −22mm (clearance) and takes the corner allowance 102mm (frame + panel) / End — Through corner: panel runs full length, normal frame" — all values recompute with the actual thickness/frame. (Naming: **Through corner** = runs full length; **Butt corner** = stops short into the neighbour. Per the confirmed engine rule the BUTTING end carries the corner allowance frame+pt — see "Corner inference" above.)
