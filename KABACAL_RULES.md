...
- **Ordem real de corte (5 ops por peça, cavidade a cavidade)**: ① POCKET `OFFSET_F` raster T12 50.8 skim (níveis 5.75/11.5, stepover 25.4, serpentina INVERTE a cada nível, penúltima linha em borda−passo/2, S12000 F9000) → ② `OFFSET_F` anel de acabamento T1 6mm a 11.8 (S18000 F8000) → ③ `OFFSET_D` V-bit 90° a 9.5 **na linha** (raio efetivo = profundidade), **entra no MEIO-DIREITO, roda HORÁRIO**, corner sharpen BR→BL→TL→TR (T6 S16000 F9000) → ④ MOULDING **sweep Ogee** T11 ball 5mm (rails `OFFSET_E` + seção `PROFILE`, compensação da esfera, passo 0.75, S15000 F10000) → ⑤ `OUT` T1: **desbastes 10.5/21 com folga +0.4** + **passe FINAL exato a 22 com ramp** (padrão VCarve, validado nos dois NCs de referência).
