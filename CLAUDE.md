...
3. **CAM / .NC output** (`ncPegasus`, `tpPartMoves`, `tpSegsForSheet`, `tpDatumOff`, `ringPts`, `emitLapFrom`, `emitRampThenLap`, `camJob`/`toolDb`/`tpDefaults`) → diff generated .NC against `tests/golden/` before/after; header `% / :1248 / G90`, toolchange block, footer `G53Z0 … M05M30`, modal coords, CRLF, datum signs and the production cutting pattern (ramp 100mm, separate last pass 0.4mm, tabs off) are validated against the real machine. A wrong .NC can crash a physical machine — be paranoid.
