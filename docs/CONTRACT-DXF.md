...
| `PROFILE` | 177 | moulding cross-section from an offset preset (factory `Ogee` or imported). Drawn ONCE per sheet (top-left, actual size, `dxfSheetProfiles`) for each profile-carrying preset used on that sheet — Doors + Panels. A **reference** for the VCarve moulding toolpath, NOT a cut. Import rules (`offsetPresetFromDxf`): `OFFSET_A`=frame-zero, B–G = inward spacing read from the DXF, `OUT` ignored |
