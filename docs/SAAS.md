...
| **1 login** | Supabase project + migration applied; optional sign-in UI (magic link); "Account" chip; signups DISABLED (invite-only) | Ednei approves D1–D4 below; isolation tests in `supabase/README.md` pass with 2 test users |
| **2 cloud jobs** | "Save to cloud / Open from cloud" next to Save/Load JSON; `job_json` = the exact `buildFastCnc()` payload | Phase 1 stable for FAST CNC itself (tenant #1) for ~a week |
