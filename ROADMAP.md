...
- **O Kabacal estava fiel ao VCarve.** Comparei `W:\Documents\Vcarve\22mm Plain Shaker.nc` (a referência de onde o template foi convertido) com o golden: sequência **estruturalmente idêntica** — rebate 2.5 / 5 / 6 e depois o passante **de 22mm numa única mordida**. Ou seja: não era erro de conversão, o defeito veio junto do template do VCarve.
- **A causa**, uma linha: `{name:'6mm Offcut 22mm', … params:{cutDepth:22, passes:1, …}}`. Depois de três passes leves (2.5/5/6), o recorte tirava **16 mm de uma vez** — exatamente o oposto de "mordidas grandes com a peça firme, passe leve no fim".
- **Correção** (números confirmados pelo Ednei): `passes:1` → `depthList:[11,21,22]` no 22mm e `[9,17,18]` no 18mm. O mecanismo já era o mesmo usado pelo rebate; nada de novo no motor.

- Testado: `node tools/check.mjs` **ok**. Nova sequência do último T1 — **22mm**: 2.5 → 5 → 6 → **11 → 21 → 22**; **18mm**: 2.5 → 5 → 6 → **9 → 17 → 18**. `Z mínimo = 0` nos dois (nada abaixo da mesa). Goldens Plain Shaker regenerados **de propósito** (2570 → 3005 e 2570 → 3004) com o README atualizado; **todos os outros goldens byte-idênticos** (RICH 18/12/9/3mm, quote 664/133/797) — só os dois arquivos do Plain Shaker mudaram.
- ⚠ **Pedidos online**: o catálogo do site é Plain Shaker 18mm → `order-intake` regerado e redeployado nesta rodada.
- Ainda pendente: a Parkin M2 para na posição errada no fim do programa (bloco de fim de programa diferente da Pegasus) — precisa de um `.nc` de referência que termine certo na Parkin.

## 2026-07-21 (g) — Quote PDF: rodapé removido + desenhos colados no cut list (fim da página em branco)

Dois pedidos do Ednei. (1) **Rodapé fora**: a linha `Generated by Kabacal · Quote valid 30 days · FAST CNC Services` saiu do `buildQuoteHtml` (só ela — o cut list interno mantém o seu "Kabacal cut list — internal."). (2) **Espaço entre o cut list e as chapas**: a causa era o CSS de impressão, não margem — `.dwg-page{break-inside:avoid}` + `@media print{.dwg-page{break-after:page}}` faziam o bloco de 3 desenhos (~240 mm) não caber no resto da página depois da tabela Parts, então o bloco INTEIRO pulava para a página seguinte e deixava o resto em branco. As duas regras foram removidas: os desenhos agora fluem logo abaixo do cut list. **`.dwg-sheet{break-inside:avoid}` fica** — nenhum desenho de chapa é partido ao meio entre páginas.

- Testado: `node tools/check.mjs` **ok**. Documento real renderizado headless antes/depois (job de 20 peças / 3 chapas, servido em localhost e aberto no Chrome): **antes** rodapé presente + `.dwg-page` com as duas regras de quebra; **depois** `rodape:"(nenhum)"`, regras ausentes, **mesmas 4 seções** (Materials & service / Additional services / Parts / Drawings) e **as mesmas 3 chapas desenhadas** — nada além disso mudou.
- **Goldens byte-idênticos** (nenhum golden cobre os docs de impressão; RICH 18/12/9/3mm, `GOLDEN_18mm.dxf`, `GOLDEN_PLAINSHAKER_S1_18mm.nc` re-verificados mesmo assim) e quote rich 664/133/797.
- Nota: as chapas deixaram de ser forçadas a 3-por-página; se algum dia isso fizer falta na impressão, é reintroduzir `break-after:page` só no bloco.
